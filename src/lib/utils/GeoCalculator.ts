...
export class GeoCalculator {
    public static getDistance(point1: { latitude: number; longitude: number }, point2: { latitude: number; longitude: number }): number {
        const R = EARTH_RADIUS; // Radius of the Earth in meters
    ...
    public static offsetTarget(target: Target, distance: number, bearing: number): { latitude: number, longitude: number } {
        ...
        let σ = s / (b * A);
        let σP = 2 * Math.PI;
        let cos2σM: number = Math.cos(2 * σ1 + σ); // Initialize before usage
        let sinσ: number = Math.sin(σ), cosσ: number = Math.cos(σ), Δσ: number;
