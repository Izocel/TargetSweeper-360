...
export class DirectionCalculator {
     ...
     */
    public static getTacticalDirection(angle: number): string {
     ...
     */
    public static getClockPosition(angle: number): { clock: string; direction: string } {
        // Normalize angle to 0-360 range
        const normalizedAngle = ((angle % 360) + 360) % 360;
        ...
        for (const dir of directions) {
            const diff = Math.min(
                Math.abs(normalizedAngle - dir.angle),
                Math.abs(normalizedAngle - (dir.angle + 360)),
                Math.abs(normalizedAngle + 360 - dir.angle)
            );
            if (diff < minDiff) {
                minDiff = diff;
                closestDir = dir;
