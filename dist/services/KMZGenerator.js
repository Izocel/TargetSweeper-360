...
class KMZGenerator {
    ...
    static fromLegacyConfig(legacyConfig) {
        const target = new Target_1.Target(legacyConfig.target.lon, legacyConfig.target.lat, legacyConfig.target.name || "Target");
        const config = new SweepConfiguration_1.SweepConfiguration(legacyConfig.diameterStep, legacyConfig.maxDiameter, legacyConfig.angleStep * 60 // Convert degrees back to MOA
        );
        return new KMZGenerator(target, config, LabelFormats_1.LabelFormat.TACTICAL);
    }
    ...
    generateSweepPlacemarks(points) {
...
exports.KMZGenerator = KMZGenerator;
//# sourceMappingURL=KMZGenerator.js.map